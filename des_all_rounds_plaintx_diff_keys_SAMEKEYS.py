...
def hex_bin(txt):
    ...
    ln = len(pc1_key1)
    gkeys = gen_keys('0F1571C947D9E859')
    #return gkeys
    r = 0
    l_key1 = []; l_key2 = []
    while r < ln:
    ...
    while nn < lg:
        idm = keysk[nn]
        shf_key1 = l_key1[idm: ]
        shf_key1 = shf_key1 + l_key1[: idm]
        shf_key2 = l_key2[idm: ]
        shf_key2 = shf_key2 + l_key2[: idm]
        ...
        nn += 1
    #return(l_pc2_key), len(l_pc2_key)
    ...
    #L0A = [chars[int(''.join(L0[i: i + 4]), 2)] for i in range(0, len(L0), 4)]
    #R0A = [chars[int(''.join(R0[i: i + 4]), 2)] for i in range(0, len(R0), 4)]
    #print(L0A, R0A)
    ...
    while n < ltp:
        ...
        Lt = Lft[n]
        Rt = Rgt[n]
        L0A = [chars[int(''.join(Lt[i: i + 4]), 2)] for i in range(0, len(Lt), 4)]
        R0A = [chars[int(''.join(Rt[i: i + 4]), 2)] for i in range(0, len(Rt), 4)]
        print(L0A, R0A)
