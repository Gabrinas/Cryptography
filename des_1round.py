...
def DES_1round(txt):
    ...
    while r < ln:
        vn1 = pc1_key1[r]
        vn2 = pc1_key2[r]
        rs1 = hex_bin(txtn)[vn1 - 1]
        rs2 = hex_bin(txtn)[vn2 - 1]
        l_key1.append(rs1)
        l_key2.append(rs2)
        r += 1
    print(l_key1, l_key2)

    # Shift the first round by 1 bit
    shf_key1 = l_key1[1: ]
    shf_key1 = shf_key1 + [l_key1[0]]
    shf_key2 = l_key2[1: ]
    shf_key2 = shf_key2 + [l_key2[0]]
    key_total = shf_key1 + shf_key2
    print(key_total)

    # PC2 Permutation Choice to get 48 bits
    ...
    while q < lr:
        vb = pc2_key[q]
        vbn = key_total[vb - 1]
        l_pc2_key.append(vbn)   # The round 1 key needed
        q += 1
    hex_equ = [int(''.join(l_pc2_key[i: i + 4]), 2) for i in range(0, len(l_pc2_key), 4)]
    e = 0
    hex_val = []
    lg = len(hex_equ)
    while e < lg:
    ...
    print(exp_res) #len(exp_res)
    print(l_pc2_key)

    # exp_res (R0 being expanded) XOR key 
    zp = list(zip(exp_res, l_pc2_key))
    ...
    # Ciphertext = L1 + R1
    L1 = R0
    L1 = [''.join(L1[i: i + 4]) for i in range(0, len(L1), 4)]

    CT = L1 + R1_gr

    CT_lang = [int(i, 2) for i in CT]
    CT_langn = ''.join([chars[i] for i in CT_lang])
    return CT_langn




print(DES_1round(txtn))
